...
class BoB(pl.LightningModule):
    """
        Bixture of BitLinears training and inference wrapper
    """
    def __init__(
            self,
            config: BoBConfig,
    ...
    def generate(
        ...
        assert hasattr(tokenizer, 'encode'), (
            f"Tokenizer {tokenizer.__name__} must have an encode method"
        )
        assert hasattr(tokenizer, 'decode'), (
            f"Tokenizer {tokenizer.__name__} must have an decode method"
        )
        ...
        unquantized_layers = [
                name for name, layer in self.model.named_modules() if isinstance(layer, BitLinear) and layer.weight_scale is None
            ]
        if len(unquantized_layers):
            logging.warning(
                'The following bitlinear layers are not quantized: %s. Inference will be done with unquantized weights.', unquantized_layers
            )
